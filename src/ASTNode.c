...
ASTNode_freeName (ASTNode_t *node)
{
  ...
  if (node->type == AST_FUNCTION || node->type == AST_NAME)
  {
...
ASTNode_setType (ASTNode_t *node, ASTNodeType_t type)
{
  ...
  if ( ( node->type == AST_FUNCTION && type == AST_NAME     ) ||
       ( node->type == AST_NAME     && type == AST_FUNCTION ) )
  {
    node->type = type;
  }
  else
