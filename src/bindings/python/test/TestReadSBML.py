...
class TestReadSBML(unittest.TestCase):
  ...
  def test_ReadSBML_line_col_numbers(self):
    ...
    sb = self.M
    if  USE_EXPAT == 1 :
      self.assert_( sb.getLine() == 3 )
      self.assert_( sb.getColumn() == 2 )
      pass    
    if  USE_LIBXML == 1 :
      self.assert_( sb.getLine() == 3 )
      pass    
    if  USE_XERCES == 1 :
      self.assert_( sb.getLine() == 3 )
      self.assert_( sb.getColumn() == 42 )
      pass    
    sb = self.M.getListOfReactions()
    if  USE_EXPAT == 1 :
      self.assert_( sb.getLine() == 4 )
      self.assert_( sb.getColumn() == 4 )
      pass    
    if  USE_LIBXML == 1 :
      self.assert_( sb.getLine() == 4 )
      pass    
