...
class TestL3FormulaParserC < Test::Unit::TestCase
  ...
  def test_SBML_C_parseL3Formula_avogadrosettings
    ...
    settings.setAvogadroCsymbol(0)    
    assert_equal false, settings.getAvogadroCsymbol()
    assert_equal false, settings.getTargetL2()
    assert_equal false, settings.getTargetL3()
    r = LibSBML::parseL3FormulaWithSettings("avogadro", settings)
    assert( r.getType() == LibSBML::AST_NAME )
    assert ((  "avogadro" == r.getName() ))
    assert( r.getNumChildren() == 0 )
    r = nil
    settings.setAvogadroCsymbol(1)
    assert_equal true, settings.getAvogadroCsymbol()
    assert_equal false, settings.getTargetL2()
    assert_equal true, settings.getTargetL3()
    r = LibSBML::parseL3FormulaWithSettings("avogadro", settings)
    assert( r.getType() == LibSBML::AST_NAME_AVOGADRO )
    assert( r.getNumChildren() == 0 )
    r = nil
  ...
  def test_SBML_C_parseL3Formula_collapseminussettings1
    ...
    r = nil
    settings = LibSBML::getDefaultL3ParserSettings()
    settings.setCollapseMinus(1)
    assert_equal true, settings.getCollapseMinus()
    r = LibSBML::parseL3FormulaWithSettings("--4.4", settings)
    assert( r.getType() == LibSBML::AST_REAL )
    assert( r.getReal() == 4.4 )
    assert( r.getNumChildren() == 0 )
    r = nil
    settings.setCollapseMinus(0)
    assert_equal false, settings.getCollapseMinus()
    r = LibSBML::parseL3FormulaWithSettings("--4.4", settings)
    assert( r.getType() == LibSBML::AST_MINUS )
    assert( r.getNumChildren() == 1 )
    c = r.getLeftChild()
  ...
  def test_SBML_C_parseL3Formula_targetl2settings
    ...
    settings = LibSBML::getDefaultL3ParserSettings()
    settings.targetL2()
    assert_equal true, settings.getTargetL2()
    assert_equal false, settings.getTargetL3()
    r = LibSBML::parseL3FormulaWithSettings("4 mL", settings)
    assert( r == nil )
    assert ((  "Error when parsing input '4 mL' at position 4:  The ability to associate units with numbers has been disabled in this software." == LibSBML::getLastParseL3Error() ))
    r = LibSBML::parseL3FormulaWithSettings("avogadro", settings)
    assert( r.getType() == LibSBML::AST_NAME )
    assert ((  "avogadro" == r.getName() ))
    assert( r.getNumChildren() == 0 )
