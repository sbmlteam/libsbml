...
beMoreSpecific($d, 'readSBML()', $file, 1);
ok($d->checkConsistency(), 0);
beMoreSpecific($d, 'checkConsistency()', $file, 1);
ok($d->getLevel(), 1);
ok($d->getVersion(), 1);
...
beMoreSpecific($d, 'readSBMLFromString()', $file, 1);
...
sub beMoreSpecific {
  ...
  $errors += $d->getNumFatals();
  $errors += $d->getNumErrors();
  $errors += $d->getNumWarnings();
  return if $errors == 0;

  print STDERR "\n=== LibSBML Messages ===\n";
  print STDERR "File:    $file\n" if defined $file;
  print STDERR "Methode: $methode\n" if defined $methode;
  $d->LibSBML::printFatals(\*STDERR);
  $d->LibSBML::printErrors(\*STDERR);
  $d->LibSBML::printWarnings(\*STDERR);
  print STDERR "========================\n\n"
}
