...
$m->addFunctionDefinition(new LibSBML::FunctionDefinition);
...
$m->addFunctionDefinition(new LibSBML::FunctionDefinition('cos'));
...
$m->addUnitDefinition(new LibSBML::UnitDefinition('volume','mmls'));
$m->addUnitDefinition(new LibSBML::UnitDefinition('mmls','volume'));
...
# add/get Compartment
$m->addCompartment(new LibSBML::Compartment);
...
# add/get Species
$m->addSpecies(new LibSBML::Species);
...
# add/get Parameter
$m->addParameter(new LibSBML::Parameter);
...
$m->addRule(new LibSBML::AssignmentRule); $Rl++;
ok($m->getNumRules(), $Rl);
...
# add/get RateRule
$m->addRule(new LibSBML::RateRule); $Rl++;
ok($m->getNumRules(), $Rl);
ok($m->getRule($Rl-1)->getTypeCode()
...
$m->addRule(new LibSBML::AlgebraicRule); $Rl++;
ok($m->getNumRules(), $Rl);
ok($m->getRule($Rl-1)->getTypeCode()
...
# add/get Reaction
$m->addReaction(new LibSBML::Reaction); $Rk++;
