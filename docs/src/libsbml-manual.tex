...
%want: a way to make the following look nicer
\item[CLASSPATH]
must include the \code{libsbmlj.jar} file from the \libsbml{}
java bindings directory.  For example:
...
%want: something more specific than "use" in this next sentence, or a green
%light to delete it
...
%want: a way to not indent this
\begin{shellVerbatim}
import org.sbml.libsbml.*;
\end{shellVerbatim}

%want: a way to indent this, so it looks like a description of the preceding
%code snippet
...
%want: nicer format for the following
...
%want: a nice way to deal with the long lines below
\begin{shellVerbatim}
>>> m = d.getModel()
>>> print m
Model:
