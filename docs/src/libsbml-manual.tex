...
libraries on Linux, Windows and MacOS X.  You will first need to make sure
one of these libraries is installed on your system.  Many Linux systems
provide one or both of these libraries either as part of their standard
distribution or as an optional RPM or Debian package.  For more
information, see \url{http://xml.apache.org/xerces-c/} for Xerces and
\url{http://expat.sf.net} for Expat.

%-----------------------------------------------------------------------------
\subsection{Linux or MacOS X}
%-----------------------------------------------------------------------------

If you have obtained the \libsbml{} source code distribution, then at your
Linux or MacOS~X command prompt, unpack the distribution, \shell{cd} into
the directory created (e.g., \shell{libsbml-2.1.0/}), and type the
...
libraries as well, add the flags \shell{--with-java}, \shell{--with-python}
and/or \shell{--with-matlab} to the configure command.  For example,
...
Windows and MacOS~X.  You will first need to make sure one of
...
The tradeoff is that Expat can be noticeably faster than Xerces at parsing
large models.
...
\libsbml{} is designed to be extremely portable.  The core is written in
100\% pure ISO~C and C++, and the build system uses GNU Autoconf and GNU
Make.
...
package by typing the following command at your Linux or MacOS~X command
prompt:
...
\subsubsection{Interfaces to Java, Python and MATLAB}
...
or work with \libsbml{} through Python's and MATLAB's interactive modes.
Short tutorials for how to use these facilities are available in the
directory \shell{docs/formatted}.

To enable the library extensions for Java, Python and MATLAB, you need to
supply additional options to configure.  For Java, use

\begin{example}[csh]
  ./configure --with-java=|\emph{dir}|
\end{example}
For Python,
\begin{example}[csh]
  ./configure --with-python=|\emph{dir}|
\end{example}
And for MATLAB,
\begin{example}[csh]
  ./configure --with-matlab=|\emph{dir}|
\end{example}

If you want to build multiple language bindings for \libsbml{}, add the
flags together.  For example,
\begin{example}[csh]
...
However, if you obtained the \libsbml{} distribution from CVS or else want
to recreate the files, you will need need SWIG, the Simplified Wrapper and
Interface Generator.  More information about SWIG is available from
\shell{http://www.swig.org}.  To tell configure to enable the use of SWIG
to regenerate the interface files, use the \shell{--with-swig} option to
configure.  
