...
\newcommand{\libsbml}{\textsc{lib\-sbml}}
...
\date{\today{}\\[10pt]\hspace*{-1ex}\libsbml{} Version \input{../../VERSION.txt}}
...
Linux, MacOS~X or Solaris command prompt, unpack the distribution,
\shell{cd} into the directory created (e.g., \shell{libsbml-2.1.0/}), and
type the following command to configure \libsbml{} for your system:
...
%=============================================================================
\section{Detailed Instructions for Configuring and Installing LibSBML}
...
both Xerces and Expat.  For more information, see the following resources:
...
to perform validation or consistency tests on SBML models, because Expat is
not a validating XML parser.  (A validating parser can check XML input
against an XML Schema.)  Xerces is currently the only validating parser
library usable with \libsbml{}.  The tradeoff is that Expat can be
noticeably faster than Xerces at parsing large models.
...
\shell{--with-java}, \shell{--with-python}, and \shell{--with-matlab}.  As
...
from \shell{http://www.swig.org}.  To tell configure to enable the use of
SWIG to regenerate the interface files, use the \shell{--with-swig} option
to configure.  If your copy of SWIG is installed in a non-standard
location, you can specify it on the \shell{configure} command line like
this:
...
\libsbml{} also implements more extensive semantic tests and consistency
checks than those represented by the XML Schema for SBML.  At the time of
this writing, over 30 tests are implemented.  Examples of these consistency
checks include: testing that compartments' \attrib{spatialSizeUnits} fields
are consistent with their \attrib{spatialDimensions}; testing that species
with \attrib{hasOnlySubstanceUnits} set to true do not also have an
