...
\shell{--enable-check} flag to the configure command:
...
forget to do a \shell{make clean} prior to the steps above.
...
\begin{methoddef}{int Species\_getHasOnlySubstanceUnits (const Species\_t *s)}
  Returns true (non-zero) if this Species' \attrib{hasOnlySubstanceUnits}
...
to, calling programs should use \libsbml{}'s \method{XXX\_isSetYYY()} class
of methods.  For \class{Species\_t}, the following are available:
...
for the corresponding \method{isSetYYY()} will state it (as above).  That
...
\class{Unit\_t} is created and (obviously) nothing is added to the model.
...
\begin{figure}[hbt]
  ...
  \caption{The definition of \class{SBase}.  See the SBML specifications
...
Finally, each SBML class has a \variable{typecode} that is initialized when
...
\class{CompartmentVolumeRule} and \class{ParameterRule}.  Having a typecode
...
KineticLaw).  Thus, the \method{KineticLaw\_getMath()} method, for example,
...
\begin{methoddef}{void SBMLDocument\_setLevel (SBMLDocument\_t *d, unsigned int level)}
  Sets the level of this SBML document to the given \variable{level}
...
\item Compartments' \attrib{spatialSizeUnits} fields must be consistent with
  their \attrib{spatialDimensions}.
  
\item Species with \attrib{hasOnlySubstanceUnits} set to true must not have
  an \attrib{initialConcentration}.
...
\begin{methoddef}{void SBMLDocument\_validate (const SBMLDocument\_t *d)}
  ...
  \method{SBMLDocument\_getNumFatals()}, and related methods.
\end{methoddef}
...
\begin{methoddef}{void * ListOf\_get (const ListOf\_t *lo, unsigned int n)}
  Returns the nth item in this list.  If n > \method{ListOf\_getNumItems(list)}
  returns \code{NULL}.
\end{methoddef}
...
\begin{methoddef}{void * ListOf\_remove (ListOf\_t *lo, unsigned int n)}
  ...
  \method{ListOf\_getNumItems(list)} returns \code{NULL}.
\end{methoddef}
