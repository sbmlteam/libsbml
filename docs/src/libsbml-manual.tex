...
%special non-indenting "verbatim" environments
\newcommand{\newVerbatimEnvNoIndent}[2]{%
\lstnewenvironment{#1}{%
\lstset{escapechar=|,%
...
\newVerbatimEnvNoIndent{cppVerbatimNoIndent}{[ANSI]C++}
\newVerbatimEnvNoIndent{cVerbatimNoIndent}{[ANSI]C}
\newVerbatimEnvNoIndent{delphiVerbatimNoIndent}{Delphi}
\newVerbatimEnvNoIndent{fortranVerbatimNoIndent}{[90]Fortran}
\newVerbatimEnvNoIndent{htmlVerbatimNoIndent}{HTML}
\newVerbatimEnvNoIndent{javaVerbatimNoIndent}{Java}
\newVerbatimEnvNoIndent{latexVerbatimNoIndent}{LaTeX}
\newVerbatimEnvNoIndent{lispVerbatimNoIndent}{Lisp}
\newVerbatimEnvNoIndent{mathematicaVerbatimNoIndent}{Mathematica}
\newVerbatimEnvNoIndent{matlabVerbatimNoIndent}{Matlab}
\newVerbatimEnvNoIndent{pascalVerbatimNoIndent}{Pascal}
\newVerbatimEnvNoIndent{perlVerbatimNoIndent}{Perl}
\newVerbatimEnvNoIndent{pythonVerbatimNoIndent}{Python}
\newVerbatimEnvNoIndent{rVerbatimNoIndent}{R}
\newVerbatimEnvNoIndent{sqlVerbatimNoIndent}{SQL}
\newVerbatimEnvNoIndent{shellVerbatimNoIndent}{csh}
\newVerbatimEnvNoIndent{tcltkVerbatimNoIndent}{[Tk]Tcl}
\newVerbatimEnvNoIndent{texVerbatimNoIndent}{[plain]TeX}
\newVerbatimEnvNoIndent{xmlVerbatimNoIndent}{XML}
...
information about SWIG is available from \shell{http://www.swig.org}.
...
the interface files, use the \shell{--with-swig} option to configure.
If your copy of SWIG is installed in a non-standard location, you can
specify it on the \shell{configure} command line like this:
...
\item[CLASSPATH]
must include the \code{libsbmlj.jar} file from the \libsbml{}
...
must include the directory that contains the \code{libsbmlj.so} file.  For
example:
...
\code{libsbmlj.jnilib} file.  For example:
...
\item[CLASSPATH]
must include the \code{sbmlj.jar} file in the Java SWIG
...
\item[PATH]
must include the directory that contains the \code{sbmlj.dll} file. An
easy way to achieve this is to copy this file to the Windows system
directory. For example:

\begin{shellVerbatim}
CD <libsbml directory>
COPY SBMLJ.DLL C:\WINDOWS\SYSTEM32
\end{shellVerbatim}
...
\begin{quote}
   Gets a \code{ListOf} object that holds references to all the
   species in the model.  A \code{ListOf} object does not operate
   the same way as a Java \code{List}.  You need to call it with
   the same method names as the C/C++ \code{ListOf} class in the
   \libsbml{} library.
\end{quote}
...
\begin{quote}
   Notice the \code{(Species)} cast on the object returned by
   \code{listOfSpecies.get()}.  As usual in Java, when extracting
...
something useful: let's convert \code{l1v1-rules.xml} from Level 1
to Level 2.

To do this, we only need to know two more things: how to
modify an \code{SBase} object, and how to write an SBML document to
...
document by calling its \code{setLevel()} method.  All \libsbml{}
objects that inherit from \code{SBase}---that is, all \libsbml{} objects
that represent entities in an SBML document---are modified by
a "setter" function whose name is \code{setXxx}, where \code{Xxx} is
the name of the data element (with the first letter
capitalized).

Then we wrote the document to a new file by calling the
static function \code{libsbml.writeSBML()}.
...
disk.  The \code{SBMLDocument}'s \code{level} data member just tells
\code{writeSBML()} the format in which to save the model.

The Level 2 version of the model is longer than the Level 1
version, so we'll only show the beginning here.  Notice that
\code{specie} got changed to \code{species} and the \code{parameterRule}s are
now expressed in MathML inside \code{assignmentRule}s:
...
First, set your \code{PYTHONPATH} environment variable to include
...
Building with \code{Python\_binding\_Win32} generates \code{\_libsbml.dll}.
\end{enumerate}
...
\item
Copy \code{\_libsbml.dll} into the windows system directory:

\begin{shellVerbatim}
C:\WINDOWS\SYSTEM32
\end{shellVerbatim}

This ensures that \code{\_libsbml.dll} is on the system \code{PATH} and
accessible anywhere.

\item
Set the environment variable \code{PYTHONPATH} to include the \libsbml{}
...
creates an \code{SBMLReader} object and names it \code{r}.  We need an
\code{SBMLReader} object in order to read in and parse an SBML
...
%-----------------------------------------------------------------------------
\subsection{Looking Inside an \class{SBase} Object}
...
\code{SBMLDocument} object.  You can retrieve the value of any data
member by calling a \code{get} function on the object.  Each
...
SBML documents are descendants of a class called \code{SBase}.  You can
\code{print} any \code{SBase} object and \code{get} the data members of any
\code{SBase} object following this same pattern.

The \code{model} data member of an \code{SBMLDocument} is itself another
\libsbml{} object.  Now we'll \code{get} the model and look inside:
...
%-----------------------------------------------------------------------------
\subsection{Modifying an \class{SBase} Object}
\label{sec:python-interactive}
%-----------------------------------------------------------------------------

For many ``getters'', there is a ``setter.''  By calling a
``setter,'' you can change an attribute of an \code{SBase} object.
For example:
...
from other information inside the \code{SBase} object.  You can't
set these:
...
To find out all the methods and data members in an \code{SBase}
object, call the Python built-in function \code{dir()}:
...
%-----------------------------------------------------------------------------
\subsection{The \class{ListOf} Class}
\label{sec:python-listof}
%-----------------------------------------------------------------------------

Several of the data members of a \code{Model} are objects of class
\code{ListOf}.  A \code{ListOf} is \libsbml{}'s internal equivalent of a
...
You can also access a \code{ListOf} using the same methods that are
...
useful: let's convert \code{l1v1-rules.xml} from Level 1 to Level 2.
...
That's all there is to it!  We changed the level of the SBML
document by calling its \code{setLevel()} method.  And then we wrote
it to a new file by calling \code{writeSBML()}.  The 1 at the end is
the value returned by \code{writeSBML()}.  A nonzero value indicates
success.
...
which the model is stored.  The \code{level} data member just tells
\code{writeSBML()} the format in which to save the model.

The Level 2 version of the model is longer than the Level 1 version, so we'll
only show the beginning here.  Notice that \code{specie} got changed to
\code{species} and the \code{parameterRule}s are now expressed in MathML
inside \code{assignmentRule}s:
...
parameter with an id of \code{tau}:
...
is a Python idiom for ``iterate the variable \code{item} through
all the elements of the list \code{items}."  \libsbml{}'s \class{ListOf}
objects, like the object returned by
\code{Model.getListOfParameters()}, are compatible with this
syntax.
...
\item
The program did an \code{import libsbml} rather than
\code{from libsbml import *}.  This is standard good form in Python, since
...
``\code{libsbml.}''  Notice the line in the program where the variable
\code{reader} is set.
...
In this case, the constructor of the \code{CreatedModel} class would
...
\code{obj[:]} or \code{copy.copy(obj)}.  The reason is that each \libsbml{}
