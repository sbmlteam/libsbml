writeErrorTable creates the html table that lists the errors and their
severities in different levels and versions.
...
Note that the spacing must be exact between the error code and the number;
the trailing comment is not necessary and is not used.
...
The resulting file should be cut and paste into SBMLError.h in place of the
existing rows within the documentation table.
