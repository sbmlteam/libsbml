...
{  if InstallOptionsPage.Values[5] then begin
  ...
  SetPreviousData(PreviousDataKey, 'Perl',    InstallOptions[3]);
  SetPreviousData(PreviousDataKey, 'Python',  InstallOptions[4]);
...
{  else if (PageID = MatlabPage.ID) and (InstallOptionsPage.Values[2] = False) then
    Result := True   }
  else if (PageID = PerlPage.ID) and (InstallOptionsPage.Values[3] = False) then
    Result := True
  else
    Result := False;
end;
...
Filename: "{app}\bindings\matlab\matlab-libSBML-4.3.0-win-libxml2-x64.exe"; Check: GetMatlab;
